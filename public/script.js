...
const THEME_MIGRATION_MAP = {
  'campaign-parchment': 'deccan-imperial',
  'lacquer-scarlet': 'crimson-crown',
  'ink-wash': 'sepia-archive',
  'miniature-brass': 'deccan-imperial',
  'sepoy-chalkboard': 'midnight-tiger',
  'rocket-night': 'midnight-tiger',
  'treaty-ledger': 'sepia-archive',
  'monsoon-glass': 'monsoon-mist',
  'coastal-signal-flags': 'royal-velvet',
  'twin-courts': 'desert-rajput',
  'emerald-mysore': 'emerald-sultan'
};

function applyTheme(themeId, save = true) {
  if (THEME_MIGRATION_MAP[themeId]) {
    themeId = THEME_MIGRATION_MAP[themeId];
  }
  ...
  if (save) {
    try {
      ...
      localStorage.setItem('tigersday_theme', themeId);
    } catch (e) {}
  }
}
...
function initTheme() {
  ...
  try {
    const stored = localStorage.getItem('tiger_day_theme') || localStorage.getItem('tigersday_theme');
    if (stored) {
      saved = THEME_MIGRATION_MAP[stored] || stored;
    }
  } catch (e) {}
